...
fn main() {
    ...
    for filename in &args[1..] {
        ...
        for tag in tags {
            ...
            tags_file.write_all(create_tag(&name, node_name, &tag, &filename).as_bytes()).unwrap();

            match node_name {
                "attr_accessor" => {
                    tags_file.write_all(create_tag(&format!("{}=", name), node_name, &tag, &filename).as_bytes()).unwrap();
                },
                "has_many" => {
                    tags_file.write_all(create_tag(&format!("{}=", name), node_name, &tag, &filename).as_bytes()).unwrap();
                    tags_file.write_all(create_tag(&format!("{}_ids", to_singular(&name)), node_name, &tag, &filename).as_bytes()).unwrap();
                    tags_file.write_all(create_tag(&format!("{}_ids=", to_singular(&name)), node_name, &tag, &filename).as_bytes()).unwrap();
                },
                "has_one" | "belongs_to" => {
                    tags_file.write_all(create_tag(&format!("{}=", name), node_name, &tag, &filename).as_bytes()).unwrap();
                    tags_file.write_all(create_tag(&format!("build_{}", name), node_name, &tag, &filename).as_bytes()).unwrap();
                    tags_file.write_all(create_tag(&format!("create_{}", name), node_name, &tag, &filename).as_bytes()).unwrap();
                    tags_file.write_all(create_tag(&format!("create_{}!", name), node_name, &tag, &filename).as_bytes()).unwrap();
                }
                _ => {}
            }
